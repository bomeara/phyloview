...
for (study.index in sequence(nrow(relevant.studies))) {
  for (tree.index in sequence(length(all.trees[study.index]))) {
    phy <- NULL
    try(phy <- get_study_tree(study_id=relevant.studies$study_ids[study.index], tree_id = all.trees[[study.index]][tree.index]))
    if(!is.null(phy)) {
      local.result <- NULL
      try(local.result <- data.frame(Year=relevant.studies$study_year[study.index], Ntax=Ntip(phy), Title=relevant.studies$title[study.index], DOI=relevant.studies$study_doi[study.index]))
